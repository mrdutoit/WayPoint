 ...
 *
 * Coercion rules the frontend FlagContext relies on: '0'/'false' -> false,
 * '1'/'true' -> true, anything else (e.g. an enum value) passes through
 * as-is. Coercion happens here, once, so every consumer sees the same
 * resolved type regardless of how the value was stored.
 */
...
export async function setTenantFlag(client, { tenantId, flagKey, valueType, value }) {
  ...
  // see the migration for why a single ON CONFLICT target can't cover both.
  if (tenantId === null) {
    await client.query(
